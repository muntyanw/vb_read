        ...
        while step < self._config.max_scroll_steps:
            ...
            for candidate in candidates:
                ...
                if self._registry.has(name):
                    ...
                    if sim:
                        log_and_print(
                            f"[personal_broadcast] skip already sent: {name} ~ {sim[0]} ({sim[1]:.2f}) scan_id={scan_id}",
                            "debug",
                        )
                    else:
                        log_and_print(
                            f"[personal_broadcast] skip already sent: {name} (exact/normalized match) scan_id={scan_id}",
                            "debug",
                        )
                    continue
                if step > 0:
    ...
    def _popup_text_confirms_no_messages(self, ok_x: int, ok_y: int, scan_id: str | None = None) -> bool:
        ...
        for m in network_markers:
            if m in norm:
                network_hits += 1
        log_and_print(
            f"[personal_broadcast] popup text OCR hits(no_messages={no_messages_hits}, network={network_hits}) "
            f"scope={scope} text='{norm[:160]}' scan_id={sid}",
            "debug",
        )
        return no_messages_hits >= 2 or network_hits >= 2
